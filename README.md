<div align="center">
  ...
  <h3>나라장터 계약 목록 Table 전체 추출 Chrome Extension</h3>
...
**Table to Excel Exporter**는 나라장터 계약 목록 페이지의 Virtual Scroll Table 전체를 자동으로 순회하여 Excel 파일로 저장하는 Chrome Extension입니다.

나라장터 계약 목록은 Virtual Scroll 방식으로 렌더링되어 화면에 보이는 Row만 DOM에 존재합니다. 이 Extension은 MutationObserver와 자동 Scroll로 전체 Data를 누락 없이 수집합니다.
...
| MutationObserver 감지 | DOM 변경을 감지하여 새 Row 렌더링 후 즉시 수집 |
| 중복 제거 | Row 번호 기준으로 중복 Data 자동 필터링 |
| 정렬 | 수집 완료 후 Row 번호 기준 오름차순 정렬 |
| Excel 저장 | 전체 Table을 `.xlsx` 형식으로 즉시 다운로드 |
...
1. 나라장터 계약 목록 페이지로 이동
2. Chrome 우측 상단 Extension 아이콘 클릭
3. **Export to Excel** 버튼 클릭
4. 전체 Data 수집 완료 후 `table_data.xlsx` 자동 다운로드
